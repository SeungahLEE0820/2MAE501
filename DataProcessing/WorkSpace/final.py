...
####### Do something ######
  ...
  #Commands
      ...
      self.Graph1 = tk.Button(self.root, text="Results Factory 1", command=self.Showplots)
      self.Graph1.place(x=1,y=90)
      
     # self.Graph2 = tk.Button(self.root, text="Results Factory 2", command=self.Showplots)
     # self.Graph2.place(x=1,y=135)
      

   #Factory 1
      self.name = self.c.create_text(750, 100, font=("Helvetica", 13), text="Command to Le Monde")
      self.Fan= tk.Button(self.root, text="Fan ON",command=self.commandGUI)
      self.Fan.place(x=720,y=120)


   #Factory 2
      self.name = self.c.create_text(750, 220, font=("Helvetica", 13), text="Command to Chocolatin")
      self.Fan2= tk.Button(self.root, text="Fan ON",command=self.commandGUIC)
...
#Check if there is something in the queue
      ...
      while True:
          if queue:
              ...
              if x[0] == "0.00":
                  ...
                  time1.append(t1)
                  if x[3] == "0": 
                      self.Alarm.config(text = "Alarm OFF", bg="green3")
                  else:
                      self.Alarm.config(text = "Alarm ON", bg="red3")
                  if x[4] == "0":
                      self.F.config(text = "Fan OFF", bg="green3")
                  else:
                      self.F.config(text = "Fan ON", bg="red3")
              if x[5] == "0.00":
                  self.Signal2.config(text = "Signal ON 2:\nNot receiving data",bg ="red2")
                  #self.Graph2.config(text = "Not able to plot")
              else:
                  self.Signal2.config(text = "Signal ON 2:\nReceiving data",bg ="green")
                  self.root.update_idletasks()
                  ...
                  time2.append(t2)
                  if x[8] == "0": 
                      self.Alarm2.config(text = "Alarm OFF", bg="green3")
                  else:
                      self.Alarm2.config(text = "Alarm ON", bg="red3")
                  if x[9] == "0":
                      self.F2.config(text = "Fan OFF", bg="green3")
                  else:
   ...
   def Showplots(self):
   	  self.root2 = tk.Tk()
   	  self.root2.title("Factories 4.0")
   	  self.canvas = tk.Canvas(self.root2, bg="gray92", height=480, width=900)
   	  f = Figure(figsize=(50,50), dpi=100)
   	  tiempo = []
   	  for i in range(0,len(self.data11)):
   	  	tiempo.append(i)
   	  plt.plot(tiempo,self.data11)
   	  canvas.pack()
   	  self.root2.mainloop()
